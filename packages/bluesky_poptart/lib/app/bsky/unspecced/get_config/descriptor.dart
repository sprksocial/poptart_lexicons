...
final liveNowConfigDescriptor = XRPCObjectDescriptor<LiveNowConfig>(
  ...
  fromJson: (json) => const LiveNowConfigConverter().fromJson(json.cast<String, dynamic>()),
  toJson: const LiveNowConfigConverter().toJson,
  matches: LiveNowConfig.validate,
);

final methodDescriptor = XRPCMethodDescriptor<EmptyData, EmptyData, UnspeccedGetConfigOutput>(
  nsid: NSID.parse('app.bsky.unspecced.getConfig'),
  kind: XRPCMethodKind.query,  outputFromJson: (json) => const UnspeccedGetConfigOutputConverter().fromJson(json.cast<String, dynamic>()),
