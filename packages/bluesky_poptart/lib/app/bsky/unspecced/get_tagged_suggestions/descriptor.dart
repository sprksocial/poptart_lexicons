...
final suggestionDescriptor = XRPCObjectDescriptor<Suggestion>(
  ...
  defName: 'suggestion',
  fromJson: (json) => const SuggestionConverter().fromJson(json.cast<String, dynamic>()),
  toJson: const SuggestionConverter().toJson,
  matches: Suggestion.validate,
);

final methodDescriptor = XRPCMethodDescriptor<EmptyData, EmptyData, UnspeccedGetTaggedSuggestionsOutput>(
  nsid: NSID.parse('app.bsky.unspecced.getTaggedSuggestions'),
  kind: XRPCMethodKind.query,  outputFromJson: (json) => const UnspeccedGetTaggedSuggestionsOutputConverter().fromJson(json.cast<String, dynamic>()),
