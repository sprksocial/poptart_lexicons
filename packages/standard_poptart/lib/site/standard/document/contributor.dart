// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, unused_import, duplicate_import, unnecessary_cast, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

import 'package:freezed_annotation/freezed_annotation.dart';
import 'package:poptart_core/poptart_core.dart';
import 'package:poptart_core/internals.dart';

part 'contributor.freezed.dart';
part 'contributor.g.dart';

// **************************************************************************
// LexGenerator
// **************************************************************************

@freezed
abstract class Contributor with _$Contributor {
  static const knownProps = <String>['did', 'role', 'displayName'];

  @JsonSerializable(includeIfNull: false)
  const factory Contributor({
    @Default('site.standard.document#contributor') String $type,
    required String did,
    String? role,
    String? displayName,

    Map<String, dynamic>? $unknown,
  }) = _Contributor;

  factory Contributor.fromJson(Map<String, Object?> json) =>
      _$ContributorFromJson(json);

  static bool validate(final Map<String, dynamic> object) {
    if (!object.containsKey('\$type')) return false;
    return object['\$type'] == 'site.standard.document#contributor';
  }
}

extension ContributorExtension on Contributor {
  bool get hasRole => role != null;
  bool get hasNotRole => !hasRole;
  bool get hasDisplayName => displayName != null;
  bool get hasNotDisplayName => !hasDisplayName;
}

final class ContributorConverter
    extends JsonConverter<Contributor, Map<String, dynamic>> {
  const ContributorConverter();

  @override
  Contributor fromJson(Map<String, dynamic> json) {
    return Contributor.fromJson(translate(json, Contributor.knownProps));
  }

  @override
  Map<String, dynamic> toJson(Contributor object) =>
      untranslate(object.toJson());
}
