...
abstract class HiddenPostsPref with _$HiddenPostsPref {
  ...
  const factory HiddenPostsPref({
    @Default('so.sprk.actor.defs#hiddenPostsPref') String $type,
    @AtUriConverter() required List<AtUri> items,

    Map<String, dynamic>? $unknown,
  }) = _HiddenPostsPref;

  factory HiddenPostsPref.fromJson(Map<String, Object?> json) => _$HiddenPostsPrefFromJson(json);

  static bool validate(final Map<String, dynamic> object) {
  if (!object.containsKey('\$type')) return false;
  return object['\$type'] == 'so.sprk.actor.defs#hiddenPostsPref'
;
}
