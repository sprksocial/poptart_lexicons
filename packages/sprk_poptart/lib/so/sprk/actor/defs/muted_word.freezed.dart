...
class _MutedWord implements MutedWord {
  const _MutedWord({this.$type = 'so.sprk.actor.defs#mutedWord', this.id, required this.value, @MutedWordTargetConverter() required final  List<MutedWordTarget> targets, @MutedWordActorTargetConverter() this.actorTarget, this.expiresAt, final  Map<String, dynamic>? $unknown}): _targets = targets,_$unknown = $unknown;
