...
abstract class JobStatus with _$JobStatus {
  static const knownProps = <String>['jobId', 'did', 'state', 'progress', 'blob', 'audio', 'error', 'message', ];

  @JsonSerializable(includeIfNull: false)
  const factory JobStatus({
    @Default('so.sprk.video.defs#jobStatus') String $type,
...
@ExtractedAudioConverter() ExtractedAudio? audio,
String? error,
String? message,
  ...
  factory JobStatus.fromJson(Map<String, Object?> json) => _$JobStatusFromJson(json);

  static bool validate(final Map<String, dynamic> object) {
  if (!object.containsKey('\$type')) return false;
  return object['\$type'] == 'so.sprk.video.defs#jobStatus'
;
}
